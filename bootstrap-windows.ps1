...
function ResolveSessionPolicyPath($mediaRoot) {
    $candidates = @()
    if (-not [string]::IsNullOrWhiteSpace([string]$mediaRoot)) {
        $candidates += Join-Path $mediaRoot "session-policy.ps1"
    }
    $candidates += Join-Path $PSScriptRoot "session-policy.ps1"
    $candidates += Join-Path $PSScriptRoot "lib\session-policy.ps1"
    ...
    throw "session-policy.ps1 was not found."
}
...
$mediaRoot = if ($media) { $media.Root } else { "" }
$sessionPolicyPath = ResolveSessionPolicyPath $mediaRoot
. $sessionPolicyPath
...
Set-WorkstationSessionPolicy | Format-Table -AutoSize | Out-String | Write-Output
